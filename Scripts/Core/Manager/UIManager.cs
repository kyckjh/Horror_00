...
public class UIManager : Singleton<UIManager>
{
    ...
    ShortCutUI[] shortCutUIs = new ShortCutUI[3];
    ...
    protected override void Awake()
    {
        ...
        shortCutUIs = shortCutPanel.GetComponentsInChildren<ShortCutUI>();
    ...
    private void Start()
    {
        ...
        OnMenuOpen += () => SetShortCutPanel(false);
        OnMenuClose += () => SetShortCutPanel(true);
    ...
    private void OnEnable()
    {
        EnableMainCanvas();
        action.UI.Enable();
        action.UI.Inventory.performed += onInventory;   // tab키 누르면 인벤토리 열림
    ...
    private void OnDisable()
    {
        ...
        action.UI.Inventory.performed -= onInventory;
        action.UI.Disable();
        if(mainCanvas != null)
        {
            DisableMainCanvas();
    ...
    {
        if(!menuPanel.activeSelf)   // 메뉴 패널이 열려있지 않을 때
        {
    ...
    public void SetShortCutPanel(bool _tf)
    {
        shortCutPanel.SetActive(_tf);
    }

    public void SetGameOverPanel(bool _tf)
    {
        gameOverPanel.SetActive(_tf);
    }

    public void SetGameClearPanel(bool _tf)
    {
        gameClearPanel.SetActive(_tf);
    }

    /// <summary>
    /// ShortCutUI의 선택된 상태 표시
    /// </summary>
    /// <param name="num">num 번째 UI 선택된 상태로 전환(0번째 부터 시작)</param>
    public void SetShortCutUI(int num)
    {
        for(int i = 0; i < 3; i++)
        {
            shortCutUIs[i].Select(i == num);
        }
    }
    ...
    /// <param name="number">활성화할 퀘스트 넘버(0 부터)</param>
